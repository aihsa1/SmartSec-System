class Message:
    def __init__(self, message, header_size, message_size=None):
        ...
        if isinstance(message, bytes):
            self.message = f"{self.message_size: <{self.header_size}}".encode() + message
        else:
    ...
    def __iadd__(self, string):
        ...
        if not((isinstance(string, str) and isinstance(self.message, str)) or (isinstance(string, bytes) and isinstance(self.message, bytes))):
            raise TypeError("The message and the string must be of the same type")
        if len(string) + len(self.message[self.header_size:]) > self.message_size:
            raise ValueError(f"the given string is too long. It should be at most {self.message_size - len(self.message[self.header_size:])}, and you gave {len(string)}")
